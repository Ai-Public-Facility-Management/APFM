...
def generate_report_node(state: dict, llm) -> dict:
    facilities = state.get("facilities", [])
    facility_text = "\n".join([
        f"- {f['name']} / {f.get('damage','N/A')} / 방해도: {f.get('hindrance_level','N/A')} / 민원: {f.get('complaints',0)}건 / 마지막 수리일: {f.get('last_repair_date','N/A')} / 견적: {f.get('estimated_cost',0)}원 / 우선순위 점수: {f.get('priority_score',0):.1f} / 근거: {f.get('cost_basis','N/A')}"
        for f in facilities
    ])
    prompt = PromptTemplate.from_template(report_prompt)
    chain = LLMChain(llm=llm, prompt=prompt)
    try:
        out = chain.invoke({"facility_list": facility_text, "inspection_date": state.get("inspection_date")})
        result_text = out.get("text") if isinstance(out, dict) else out
    except Exception:
        result_text = chain.run(facility_text)
    state["final_report"] = result_text
    return state
